...
import { PersonRepository, getServiceClient } from '@sobremesa/database';
import {
 ...
 */
export const identityRoutes = new Elysia({ prefix: '/api/family' })
  .use(authPlugin)
  /**
  ...
  .get(
    ...
    async ({ params: { familyId }, auth, set }) => {
      ...
      let currentClaim: (PersonSuggestion & { notes?: string | null }) | null =
        null;
      if (access?.personId) {
      ...
      if (displayName && !currentClaim) {
        const match = await personRepo.findBestMatch(familyId, displayName, []);
        if (match) {
          suggestion = {
            id: match.person.id,
      ...
      const topPeople: PersonSuggestion[] = (topPeopleData || []).map((p) => ({
        id: p.id,
        name: p.name,
        aliases: p.aliases || [],
