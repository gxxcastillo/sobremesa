/**
 * Authz boundary tests for `adminRoutes` (`.use(requireSuperAdmin)` on the
 * whole file).
 */
...
// Representative route: GET /api/chats. No params or body at all.
const url = 'http://localhost/api/chats';

describe('adminRoutes authz (requireSuperAdmin)', () => {
  ...
  it('returns 403 for an authenticated non-super-admin', async () => {
    const spec = {
      ...
      familyAccess: [{ familyId: 'family-1', role: 'admin' as const }],
    };
